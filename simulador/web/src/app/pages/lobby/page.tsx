...
const LobbyPage = () => {
  ...
  useEffect(() => {
    ...
    // Conexão do Socket ao namespace /simulation
    ...
    // Atualiza config em tempo real (admin mudou duração/rodada)
    ...
    // ✅ GATILHO GLOBAL: round:started → salva dados e navega para /simulattion
    socket.on('round:started', (data: { round: number; duration: number; endTime: string }) => {
      // Salvar dados da rodada para a tela de simulação ler
      ...
      setRoundLabel(`Rodada ${data.round} começando!`);

      // Pequeno delay para exibir animação de "prepare-se" antes de navegar
      setTimeout(() => {
        router.push('/pages/simulattion');
      }, 1800);
    });

    // Sincronização inicial da lista de players já conectados
    fetch(`${API_URL}/minigame/session/${sessionId}`)
      .then(res => res.json())
      .then(data => {
        if (data.players) setPlayers(data.players);
        if (data.adminName) setConfig(prev => ({ ...prev, adminName: data.adminName }));
      })
  ...
  // Timer regressivo (apenas visual no lobby)
  useEffect(() => {
  ...
  const handleConfirmReady = () => {
    ...
    socketRef.current.emit('player:ready', { playerId: myPlayerData.id });
    setIsReady(true);
  };

  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 lg:p-8 font-sans selection:bg-orange-500 relative">
      ...
      <AnimatePresence>
        {isGameStarted && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center bg-[#002350]/90 backdrop-blur-sm"
          >
            <div className="text-center space-y-4">
              ...
              <p className="text-orange-500 font-black text-xl uppercase tracking-widest">
                {roundLabel}
              </p>
              <p className="text-white/50 text-xs font-bold uppercase tracking-widest animate-pulse">
                Carregando tela de decisões...
              </p>
            </div>
          </motion.div>
        )}
      ...
      <div className="max-w-7xl mx-auto mb-8 flex flex-col md:flex-row justify-between items-center gap-6">
        <motion.div
          ...
          className="flex items-center gap-4 bg-white px-6 py-3 rounded-2xl shadow-sm border border-gray-100 border-b-4 border-blue-900"
        >
          ...
          <div className="flex flex-col">
            ...
            <span className="text-sm font-black text-blue-900 uppercase italic">#{myPlayerData?.sessionId?.slice(-8).toUpperCase() || '---'}</span>
          </div>
        </motion.div>
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-8 rounded-[3rem] shadow-sm border border-gray-100">
            ...
            <div className="space-y-3 max-h-[350px] overflow-y-auto pr-2 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-slate-200 [&::-webkit-scrollbar-thumb]:rounded-full">
              {players.map((p) => (
                <div
                  key={p.id}
                  className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${p.id === myPlayerData?.id ? 'bg-blue-50 border-blue-900/10' : 'bg-gray-50 border-transparent opacity-60'}`}
                >
                  <div className="flex flex-col">
          ...
          <div className="bg-blue-900 p-10 rounded-[3rem] text-white relative overflow-hidden shadow-2xl border-b-[10px] border-orange-500">
            ...
            <p className="relative z-10 text-blue-100 text-xs font-bold leading-relaxed opacity-70 mb-4">
              Fase {config.round}: Otimização de KPIs e Gestão de Fluxo. Siga as instruções do facilitador{' '}
              <span className="text-orange-500">{config.adminName}</span>.
            </p>
        ...
        <div className="lg:col-span-8">
          <motion.div
            ...
            animate={{ opacity: 1, scale: 1 }}
            className="bg-white p-8 lg:p-14 rounded-[3.5rem] shadow-sm border border-gray-100 h-full relative"
          >
            <div className="mb-12">
