...
export const StatsMetrics = ({ players, connected }: StatsMetricsProps) => {
  ...
  return (
    <section className="grid grid-cols-1 md:grid-cols-3 gap-5">
      ...
      {/* PLAYERS (Borda trocada para Cinza) */}
      <div className="bg-[#0d121f] border border-slate-800/60 rounded-3xl p-6 shadow-xl">
      ...
      {/* READY (Borda trocada para Cinza) */}
      <div className="bg-[#0d121f] border border-slate-800/60 rounded-3xl p-6 shadow-xl">

        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-orange-500/10 border border-orange-500/40 flex items-center justify-center">
        ...
        <div className="mt-5 h-2 bg-black/40 rounded-full overflow-hidden border border-white/[0.05]">
          <motion.div
      ...
      {/* CONNECTION (Borda trocada para Cinza) */}
      <div className="bg-[#0d121f] border border-slate-800/60 rounded-3xl p-6 shadow-xl">

        <div className="flex items-center gap-4">
          <div
            className={`w-12 h-12 rounded-2xl flex items-center justify-center border ${
              connected ? "bg-emerald-500/10 border-emerald-500/40" : "bg-red-500/10 border-red-500/40"
            }`}
          >
            {connected ? <Wifi className="text-emerald-400" size={20} /> : <WifiOff className="text-red-400" size={20} />}
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-[0.25em] text-slate-500 font-black">
              Conexão
            </p>
            <p className={`text-lg font-black mt-1 uppercase ${connected ? "text-emerald-400" : "text-red-400"}`}>
