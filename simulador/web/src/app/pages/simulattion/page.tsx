...
const SimulacaoPage = () => {
  ...
  useEffect(() => {
    ...
    // ✅ Próxima rodada iniciada pelo admin → reinicia tela com novos dados
    socket.on('round:started', (data: { round: number; duration: number; endTime: string }) => {
      // Atualiza dados da rodada no localStorage
      ...
      // Reinicia todos os estados para a nova rodada
      setRoundData({ roundNumber: data.round, duration: data.duration, endTime: data.endTime });
      setIsSubmitted(false);
      setIsTimeUp(false);
      setSelectedCapex(new Set());

      // Reinicia timer com novo endTime
      if (timerRef.current) clearInterval(timerRef.current);
      startTimer(data.endTime);

      // Reinicia stock inputs zerados
      setStockInputs(prev => {
        const reset: Record<string, StockInput> = {};
        Object.keys(prev).forEach(id => {
          reset[id] = { ...prev[id], buyQty: 0 };
        });
        return reset;
      });

      toast(`🚀 Rodada ${data.round} iniciada!`, {
        style: { background: '#f97316', color: '#fff', fontWeight: 'bold' },
        duration: 3000,
      });
    });
    ...
    socket.on('simulation:finished', (data: { rankData: any[] }) => {
      if (data?.rankData) localStorage.setItem('rank_data', JSON.stringify(data.rankData));
  ...
  if (isSubmitted) {
    return (
      <div className="min-h-screen bg-[#002350] flex flex-col items-center justify-center gap-8 p-6">
        ...
        <p className="text-white/20 text-[10px] font-black uppercase tracking-widest text-center max-w-xs">
          Quando o admin iniciar a próxima rodada, a tela atualizará automaticamente
        </p>
      </div>
    );
  }
